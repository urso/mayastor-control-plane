...
mod diskpool_tests {
    ...
    fn test_single_disk_translation() {
        ...
        let raid_config = spec.raid.as_ref().map(|r| r.clone().into());
        let body = CreatePoolBody::new_all(spec.disks(), None, None, None, raid_config);
    ...
    fn test_multiple_disks_raid0_translation() {
        ...
        let mut spec = DiskPoolSpec::new(
            "worker-1".to_string(),
            vec!["/dev/sda".to_string(), "/dev/sdb".to_string()],
            None,
            None,
        );
        spec.raid = Some(RaidConfig::Raid0(raid0_config));
